...
class Detector: SCNNode {
    ...
    private var recentDetectorPositions: [SIMD3<Float>] = []
    
    /// Previously visited plane anchors.
    private var anchorsOfVisitedPlanes: Set<ARAnchor> = []
    
    /// List of the segments in the focus square.
    private var segments: [Detector.Segment] = []
    
    /// The primary node that controls the position of other `Detector` nodes.
    ...
    override init() {
        ...
        let c: Float = Detector.thickness / 2 // correction to align lines perfectly
        ...
        positioningNode.simdScale = [1.0, 1.0, 1.0] * (Detector.size * Detector.scaleForClosedSquare)
    ...
    func setPosition(with raycastResult: ARRaycastResult, _ camera: ARCamera?) {
        ...
        recentDetectorPositions.append(position)
    ...
    private func updateTransform(for raycastResult: ARRaycastResult, camera: ARCamera?) {
        ...
        recentDetectorPositions = Array(recentDetectorPositions.suffix(10))
        
        // Move to average of recent positions to avoid jitter.
        let average = recentDetectorPositions.reduce([0, 0, 0], { $0 + $1 }) / Float(recentDetectorPositions.count)
    ...
    private func performOpenAnimation() {
        ...
        SCNTransaction.animationDuration = Detector.animationDuration / 4
        ...
        SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: .easeOut)
        SCNTransaction.animationDuration = Detector.animationDuration / 4
        positioningNode.simdScale = [1.0, 1.0, 1.0] * Detector.size
        SCNTransaction.commit()
    }
    ...
    private func performCloseAnimation(flash: Bool = false) {
        ...
        SCNTransaction.begin()
        SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: .easeOut)
        SCNTransaction.animationDuration = Detector.animationDuration / 2
        ...
        if flash {
            let waitAction = SCNAction.wait(duration: Detector.animationDuration * 0.75)
            let fadeInAction = SCNAction.fadeOpacity(to: 0.25, duration: Detector.animationDuration * 0.125)
            let fadeOutAction = SCNAction.fadeOpacity(to: 0.0, duration: Detector.animationDuration * 0.125)
            fillPlane.runAction(SCNAction.sequence([waitAction, fadeInAction, fadeOutAction]))
            
            let flashSquareAction = flashAnimation(duration: Detector.animationDuration * 0.25)
            for segment in segments {
    ...
    private func scaleAnimation(for keyPath: String) -> CAKeyframeAnimation {
        ...
        let size = Detector.size
        let ts = Detector.size * Detector.scaleForClosedSquare
        ...
        scaleAnimation.duration = Detector.animationDuration
    ...
    private lazy var fillPlane: SCNNode = {
        let correctionFactor = Detector.thickness / 2 // correction to align lines perfectly
        let length = CGFloat(1.0 - Detector.thickness * 2 + correctionFactor)
        ...
        material.diffuse.contents = Detector.fillColor
        material.isDoubleSided = true
        material.ambient.contents = UIColor.black
        material.lightingModel = .constant
        material.emission.contents = Detector.fillColor
    ...
    */
    ...
    class Segment: SCNNode {
        ...
        init(name: String, corner: Corner, alignment: Alignment) {
            ...
            let material = plane.firstMaterial!
            material.diffuse.contents = Detector.primaryColor
            material.isDoubleSided = true
            material.ambient.contents = UIColor.black
            material.lightingModel = .constant
            material.emission.contents = Detector.primaryColor
            geometry = plane
        }
